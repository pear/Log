...
``false``.  Otherwise, it should return ``true``.  The ``$this->opened``
...
string format($format, $timestamp, $priority, $message)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
``announce()`` should be called from a handler's `log()`_ method whenever an
